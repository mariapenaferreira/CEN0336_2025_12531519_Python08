...
if len(sys.argv) != 2:
    print("Uso: python3 Python08_6.py Python_08.fasta")
    sys.exit(1)
...
# Função para encontrar o peptídeo mais longo.
def encontrar_peptideo_mais_longo(aa_seq):
    candidatos = []
    atual = ""
    dentro = False
    for aa in aa_seq:
...
with open("Python_08.translated-longest.aa", "w") as out:
    for name, seq in seqs.items():
        ...
        for strand, s in [("forward", seq), ("reverse", rev_seq)]:
            for frame in range(3):
                ...
                pep = encontrar_peptideo_mais_longo(aa_seq)
                if len(pep) > len(mais_longo):
                    mais_longo = pep
        out.write(f">{name}\n{mais_longo}\n")

print("Peptídeos mais longos dos seis quadros gravados em 'Python_08.translated-longest.aa'")
